...
export default function CustomizedMenus(props) {
    ...
    try {
        if(props.is_nbaNfl === 'NBA'){
            if (props.salary === "dk") {
                let DK_keys = Object.keys(props.nba_slate_data['DK'])
                selectMenu = <FormControl fullWidth>
                    <InputLabel id="demo-simple-select-label" sx={{marginTop: -1}}> Filter By Slates</InputLabel>
                    <Select
                        labelId="demo-simple-select-label"
                        id="demo-simple-select"
                        style={{width: 273,padding:'0px'}}
                        onChange={ChangeData}
                    >
                        {DK_keys.map((el) => (
                            <MenuItem value={el}>
                ...
                console.log(Object.keys(props.slate_data['FD']))
                let FD_Keys = Object.keys(props.nba_slate_data['FD'])
                selectMenu = <FormControl fullWidth>
                    <InputLabel id="demo-simple-select-label" sx={{marginTop: -1}}> Filter By Slates</InputLabel>
                    <Select
                        labelId="demo-simple-select-label"
            ...
            if (props.salary === "dk") {
                let DK_keys = Object.keys(props.slate_data['DK'])
                selectMenu = <FormControl fullWidth>
                    <InputLabel id="demo-simple-select-label" sx={{marginTop: -1}}> Filter By Slates</InputLabel>
                    <Select
                        labelId="demo-simple-select-label"
                ...
                console.log(Object.keys(props.slate_data['FD']))
                let FD_Keys = Object.keys(props.slate_data['FD'])
                selectMenu = <FormControl fullWidth>
                    <InputLabel id="demo-simple-select-label" sx={{marginTop: -1}}> Filter By Slates</InputLabel>
                    <Select
                        labelId="demo-simple-select-label"
    ...
    <StyledMenuItem>
        <div className="menu_list_head">
            <div className="team_container row">
                <div data-awayName={`left${gameData.Id}`} id={`left${gameData.Id}`} awayTeamName={gameData.AwayTeam}
                     className={`left_team ${active}${props.is_nbaNfl === 'NFL' ? "NFL" + gameData.GlobalAwayTeamID : "NBA" + gameData.AwayTeamID} col-md-5 col-sm-5 col-xs-5`}
                     onClick={(e) => {
                         activeDeactive(e, gameData.AwayTeam)
                     }} style={{"padding-top": "3%"}}>
                    {props.is_nbaNfl === 'NFL' ?
                        <div data-awayName={`left${gameData.Id}`}
                             className={`${"NFL" + gameData.GlobalAwayTeamID} nfl`}/>

                        : <div data-awayName={`left${gameData.Id}`} className={`${"NBA" + gameData.AwayTeamID}`}/>}
                    {gameData.AwayTeam}
                </div>
                <div data-middleName={`middle${gameData.Id}`} id={`middle${gameData.Id}`}
                     awayTeamName={gameData.AwayTeam} at={"@"} homeTeamName={gameData.HomeTeam}
                     className="middle_team active  col-md-2 col-sm-2 col-xs-2" onClick={(e) => {
                    activeDeactive(e, gameData.AwayTeam, gameData.HomeTeam)
                }}>@
                </div>
                <div data-homeName={`right${gameData.Id}`} id={`right${gameData.Id}`} homeTeamName={gameData.HomeTeam}
                     className={`right_team ${active}${props.is_nbaNfl === 'NFL' ? "NFL" + gameData.GlobalHomeTeamID : "NBA" + gameData.HomeTeamID}  col-md-5 col-sm-5 col-xs-5`}
                     onClick={(e) => {
                         activeDeactive(e, gameData.HomeTeam)
                     }} style={{"padding-top": "3%", "padding-left": "0px"}}>
                    {props.is_nbaNfl === 'NFL' ?
                        <div data-homeName={`right${gameData.Id}`}
                             className={`${"NFL" + gameData.GlobalHomeTeamID} nfl`}/>
    ...
    <div>
        ...
        <div className="class_head">
            <StyledMenu
                ...
                onClose={handleClose}
            >
                {/*{props.is_nbaNfl === 'NFL' ? selectMenu : null}*/}
                {selectMenu}
                        ...
                        <div>
                            ...
                            <div className="row mr-0">
                                ...
                                <div className="right_des  col-md-5 col-sm-5 col-xs-5">
                                    <span
                                        className="text-uppercase">{props.HomeTeam} {props.spread > 0 ? "+" + props.spread : props.spread}</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <>
                                    <li className="collapse_single">
                                        ...
                                        <div className={"gameDataButG"}>
                                            <center>
                                                <span className={`gameDataBut`} id={props.id + "Winner1"}
                                                      onClick={() => {
                                                          fff(props.id, props.AwayTeam, "Winner", props.id + "Winner1")
                                                      }}> {props.AwayTeam} </span>
                                                <span className={`gameDataButt`} id={props.id + "Winner2"}
                                                      onClick={() => {
                                                          fff(props.id, props.HomeTeam, "Winner", props.id + "Winner2")
                                                      }}> {props.HomeTeam} </span>
                                            </center>
                                        </div>
                                    </li>
                                    <li className="collapse_single">
                                        ...
                                        <div className={"gameDataButG"}>
                                            <center>
                                                <span className={`gameDataBut`} id={props.id + "PointSpreadHome1"}
                                                      onClick={() => {
                                                          fff(props.id, false, "PointSpreadHome", props.id + "PointSpreadHome1")
                                                      }}> {(-1 * props.spread) > 0 ? "+" + (-1 * props.spread) : (-1 * props.spread)} </span>
                                                <span className={`gameDataButt`} id={props.id + "PointSpreadHome2"}
                                                      onClick={() => {
                                                          fff(props.id, true, "PointSpreadHome", props.id + "PointSpreadHome2")
                                                      }}> {props.spread > 0 ? "+" + (props.spread) : props.spread} </span>
                                            </center>
                                        </div>
                                    </li>
                                    <li className="collapse_single">
                                        ...
                                        <div className={"gameDataButG"}>
                                            <center>
                                                <span className={`gameDataBut`} id={props.id + "OverTotal1"}
                                                      style={{padding: "6px 6px"}} onClick={() => {
                                                    fff(props.id, true, "OverTotal", props.id + "OverTotal1")
                                                }}> {"OVER " + parseFloat(props.overUnder)} </span>
                                                <span className={`gameDataButt`} id={props.id + "OverTotal2"}
                                                      style={{padding: "6px 1px"}} onClick={() => {
                                                    fff(props.id, false, "OverTotal", props.id + "OverTotal2")
                                                }}> {"UNDER " + parseFloat(props.overUnder)} </span>
                                            </center>
                                        </div>
                                    </li>
                                </>
